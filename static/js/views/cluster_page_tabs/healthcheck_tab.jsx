...
function($, _, i18n, Backbone, React, models, utils, componentMixins, controls) {
    ...
    var HealthCheckTab = React.createClass({
        mixins: [
            componentMixins.backboneMixin({
                //React.backbone插件就是实现backbone中数据发生变化时,调用react的render函数
                //当cluster下面的tasks集合有添加task,删除task,改变task的状态时候
                //调用组件的render方法
                //React关联backbone的collection
                modelOrCollection: function(props) {return props.model.get('tasks');},
                renderOn: 'add remove change:status'
            }),
            //React关联backbone的model
    ...
    var HealthcheckTabContent = React.createClass({
        ...
        isLocked: function() {
            ...
            //如果集群是新建状态或者集群正在部署,那么健康检测页面不可操作
        ...
        runTests: function() {
            ...
            if (testruns.length) {
                ...
                //这相当于调用ostf中TestrunsController的post函数
            }
            if (oldTestruns.length) {
                requests.push(Backbone.sync('update', oldTestruns));
                //这相当于调用ostf中TestrunsController的put函数
            }
            $.when.apply($, requests)
                .done(_.bind(function() {
    ...
    var HealthcheckCredentials = React.createClass({
        ...
        render: function() {
            ...
            var cred=this.props.credentials;
            if(this.props.cluster.get('cluster_type')!="1")
            {
              cred['user']='admin';
