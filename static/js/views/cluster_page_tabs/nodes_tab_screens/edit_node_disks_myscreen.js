...
function($, _, i18n, Backbone, utils, models, EditNodeScreen, editNodeDisksScreenTemplate, nodeDisksTemplate, volumeStylesTemplate) {
    ...
    EditNodeDisksScreen = EditNodeScreen.extend({
        ...
        hasChanges: function() {
            ...
            var nodescollection=new models.Nodes(app.nodeModel);
            return !nodescollection.reduce(function(result, node) {
        ...
        loadDefaults: function() {
            ...
            var nodescollection=new models.Nodes(app.nodeModel);
            this.disks.fetch({url: _.result(nodescollection.at(0), 'url') + '/disks/defaults/'})
        ...
        revertChanges: function() {
            //取消变更
             var nodescollection=new models.Nodes(app.nodeModel);
            this.disks.reset(_.cloneDeep(nodescollection.at(0).disks.toJSON()), {parse: true});
        },
        applyChanges: function() {
            //执行完此函数后this.nodes会发生变化，this.nodes变成列表页面所有的node
            if (this.hasValidationErrors()) {
                return (new $.Deferred()).reject();
            }
            var nodescollection=new models.Nodes(app.nodeModel);
            this.disableControls(true);
            return $.when.apply($, nodescollection.map(function(node) {
        ...
        render: function() {
            this.$el.html(this.template({
                nodes: new models.Nodes(app.nodeModel),
