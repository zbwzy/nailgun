 ...
 define(
  [
    ...
    'use strict';
     ...
     var HealthCheckTab=Backbone.createClass({
         mixins:[
     ...
     var HealthCheckTabContent =React.createClass({
        ...
        getNumberOfCheckedTests:function(){
          return this.props.tests.where({checkd:true}).length;
        },
        toggelCredentials:function(){
          this.setState({credentialsVisible:!this.state.credentialsVisible});
        },
        handleSelectAllClick:function(name,value)
        {
          //和underscore中的_invoke效果一样
          //在list的每个元素上执行methodName方法,循环调用backbone的model的set方法
          //批量改变list中的每一个model的属性值
              ...
              <div>
                 <div className='row-fluid page-sub-title'>
                    <div className='span2 ostf-controls'>
                       {!disabledState &&
                         <div className='toggle-credentials pull-right' onClick={this.toggelCredentials}}>
                          ...
                          <thead>
                            ...
                            <tbody>
                              {this.props.tests.map(function(test){
                                  ...
                                  var status=result && result.status || 'unknow';
                                  return <Test
                                            key={test.id}
                                            test={test}
                                            result={result}
     ...
     var Test=React.createClass({
        mixins:[componentMixins.backboneMixin('test')
        ],
