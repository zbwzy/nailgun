...
function($, _, i18n, Backbone, React, utils, models, controls) {
    ...
    dialogs.DeployChangesDialog = React.createClass({
        ...
        setClusterRolesStatus:function()
        {
          var roleobjs=this.props.cluster.get('release').get('role_models');
          for(var t=0;t<roleobjs.length;t++)
          {
            var cansetrole=roleobjs.models[t]?roleobjs.models[t].get('can_start_down'):0;
            var roletype=roleobjs.models[t].get('cluster_type');
            if(cansetrole>0 &&(roletype==this.props.cluster.get('cluster_type')))
            {
              var rolename=roleobjs.models[t].get('name');
              ...
              $.ajax({
                ...
                success: _.bind(function(msg){
                    console.log("初始化集群角色状态成功!");
                },this),
                error:function(msg)
                {
                   console.log("初始化集群角色状态失败!");
                }
              });
            }
          }
        ...
        renderChange: function(change, nodeIds) {
            ...
            return (
                <div key={change}>
                    <div className='deploy-task-name'>{i18n('dialog.display_changes.settings_changes.' + change,{env:this.getClusterTypeName(this.props.cluster.get('cluster_type'))})}</div>
                    <ul>
        ...
        getClusterTypeName:function(type){
          if (type==1 || type==4)  return "OpenStack";
          if (type==2)             return "CloudMaster";
          if (type==3)             return "EBS";
          if (type==5)             return "ONEST";
          return "未知环境";
        },
        renderBody: function() {
            ...
            return (
                <div className='display-changes-dialog'>
                    ...
                    <div className='amount-restrictions'>
                        {(cluster.get('cluster_type')==1 && this.state.amountRestrictions.controller) &&
                            <div className='alert alert-error'>{i18n(ns + 'warnings.controller', {count: requiredNodeAmount})}</div>
                        }
                        {(cluster.get('cluster_type')==1 && this.state.amountRestrictions.compute) &&
                            <div className='alert alert-error'>{i18n(ns + 'warnings.compute')}</div>
                        }
                        {(cluster.get('cluster_type')==1 && this.state.amountRestrictions.mongo) &&
    ...
    dialogs.ShowMyNodeInfoDialog = React.createClass({
        mixins: [dialogMixin],
        goToConfigurationScreen: function(url) {
          this.close();
          app.nodeModel=this.props.node;
           if(url=="disks")
        ...
        renderFooter: function() {
            ...
            return (
                <div>
                    <span>
                            <button className='btn btn-edit-networks' onClick={this.goToConfigurationScreen.bind(this, 'interfaces')}>{i18n('dialog.show_node.network_configuration_button')}</button>
                            <button className='btn btn-edit-disks' onClick={this.goToConfigurationScreen.bind(this, 'disks')}>磁盘查看</button>
                    </span>
                    <button className='btn' onClick={this.close}>{i18n('common.cancel_button')}</button>
                </div>
            );
        ...
        deleteNodes: function() {
            ...
            this.props.nodes.each(function(node) {
                var data = !node.get('pending_addition') ? {pending_deletion: true} : {
                        ...
                        pending_roles: [],
                        node_type:this.props.cluster.get('cluster_type')
                    };
