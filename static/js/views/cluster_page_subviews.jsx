...
function(_, i18n, React, utils, dialogs, componentMixins) {
    ...
    clusterPageSubviews.ClusterInfo = React.createClass({
        ...
        render: function() {
            ...
            var canshow="";
            var releasetitle="OpenStack版本";
            if(cluster.get('cluster_type')!="1")
            {
              canshow="none";
              releasetitle="系统版本";
            }
    ...
    clusterPageSubviews.DeploymentControl = React.createClass({
        ...
        render: function() {
            ...
            var hasSaveSetting=true;
            var settingmessage="";
                if(cluster.get('cluster_type')!="1" )
                {
                    if(!cluster.get('cluster_setting'))
                    {
                       hasSaveSetting=false;
                    } 
                   ...
                   if(msgflag==false)
                    {  
                        if(hasSaveSetting==false)
                        {
                          if(this.state.hidden==false) {
                              settingmessage="请先保存环境设置信息,然后再部署变更";
                          }
                        }
                    }
                }
                    ...
                    <div className='pull-right deployment-control-box'>
                        <div className={itemClass}>
                            <button
                                ...
                                disabled={cluster.get('release').get('state') != 'available' || (!cluster.hasChanges() && !cluster.needsRedeployment()) || nodes.every({status: 'ready', pending_deletion: false}) ||(!hasSaveSetting)}
                        ...
                        <div id="showsetmsg" style={{paddingTop:"10px",color:'#cc2229',fontSize:"16px",width:"300px",float:"right",paddingRight:"20px"}}>{settingmessage}</div>
                    </div>
                );
        }
    });
