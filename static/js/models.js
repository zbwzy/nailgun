...
define([
    ...
    models.Node = Backbone.Model.extend({
        ...
        getRolesSummary2: function(releaseRoles) {
            //this.sortedRoles(releaseRoles.pluck('name')) rolename的数组
            return _.map(this.sortedRoles(releaseRoles.pluck('name')), function(role) {
                return releaseRoles.findWhere({name: role}).get('label');
                //循环name数组，返回releaseRoles角色对象集合的name和当前name相同的label
            }).join(', ');
        },
         getRolesSummary: function(releaseRoles,roles) {
             return _.map(this.sortedRoles(releaseRoles.pluck('name')), function(role) {
                return releaseRoles.findWhere({name: role}).get('label');
            }).join(', ');
        },
    ...
    models.Nodes = Backbone.Collection.extend({
        ...
        comparator: function(node1, node2) {
            return utils.multiSort(node1, node2, [{attr: 'online', desc: true}, {attr: 'id',desc:true}]);
        },
        hasChanges: function() {
