...
from nailgun.orchestrator import myprovisioning_serializers
...
class CheckBeforeDeploymentTask(object):
    ...
    def execute(cls, task):
        ...
        if task.cluster.cluster_type==1:
...
class InstallosTask(object):
    ...
    def message(cls, task, nodes_to_provisioning):
        ...
        objects.NodeCollection.lock_nodes(nodes_to_provisioning)
        serialized_nodes = myprovisioning_serializers.serialize(
           nodes_to_provisioning)
        ...
        rpc_message = make_astute_message(
            ...
            'provision_resp',
            {
                'provisioning_info': serialized_nodes
            }
        )
        db().commit()
        return rpc_message
