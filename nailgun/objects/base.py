...
class NailgunCollection(object):
    ...
    @classmethod
    def getPagingData(cls,currentpage,pagesize,ordername,ordertype):
        """
          返回一个分页而且排序的数据列表
        """
        if currentpage=="0":
          #获取列表总条数,首次加载列表的数据从这里取得
          q=cls.all()
          if ordertype=="desc":
            q=cls._query_order_by(q,('-'+ordername,))
          else:
            q=cls._query_order_by(q,(ordername,))
        else:
          #按照分页条件取出对应的数据
          ...
          endPosition=pageIndex*pageSize

          q=cls.all()
          #在列名前面加上-代表按这个字段倒叙,否则按照正序排列
          if ordertype=="desc":
            q=cls._query_order_by(q,('-'+ordername,))
          else:
            q=cls._query_order_by(q,(ordername,))
          maxEndPosition=len(list(q))
          if endPosition>maxEndPosition:
             endPosition=maxEndPosition
          q=q.slice(startPosition,endPosition)

        return cls.to_json(q)