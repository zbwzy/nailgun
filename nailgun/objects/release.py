...
class Release(NailgunObject):
    ...
    def update_roles(cls, instance, roles):
        ...
        added_roles = instance.roles #added_roles表示已经添加过的角色,不再添加
        for role in roles:
            if role not in added_roles:
                b=role.find('_')
                #这些角色只针对特定的集群类型
                rolename=role[:-2]
                clustertype=int(role[-1])
                if clustertype==2 and instance.id==2: #cloudmaster不支持ubuntu
                    continue
                else:
