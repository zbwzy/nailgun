...
#因为循环导入依赖的问题,task中不能直接导入objects下的node
...
class Task(NailgunObject):
    ...
    def _update_parent_instance(cls, instance):
        ...
        if len(subtasks):
            ...
            if all(map(lambda s: s.status == 'ready', subtasks)):
                sort_subtasks = sorted(subtasks,key=lambda s:s.timestamp, reverse=False)
                data['status'] = 'ready'
                data['progress'] = 100
                data['message'] = u'\n'.join(map(
                    lambda s: s.message, filter(
                        lambda s: s.message is not None, sort_subtasks)))
    ...
    @classmethod
    def _update_nodes_data(cls, instance):
        node_ids=str(instance.node_ids.lstrip('{').rstrip('}'))
        ids=node_ids.split(',')
        to_update=db().query(Node).filter(Node.id.in_(ids))
        data={}
        if instance.status=='error':
                data['status'] =consts.NODE_STATUSES.error
                data['progress'] = 100
                data['error_type'] = 'provision'
                data['error_msg'] =instance.message 
        else:           
                data['status'] = instance.status
                data['progress'] = instance.progress
        to_update.update(data,synchronize_session=False)
        db().flush()
    ...
    def update(cls, instance, data):
        ...
        if instance.cluster_id:
            ...
            if instance.status=="error":
               logger.debug('updating node ids :%s', instance.node_ids)
               cls._update_nodes_data(instance)
