...
class NailgunReceiver(object):
    ...
    def remove_nodes_resp(cls, **kwargs):
        ...
        if nodes:
            success_msg = u"成功删除 {0} node(s)".format(
                len(nodes)
            )
            notifier.notify("done", success_msg)
        if error_nodes:
            err_msg = u"删除失败 {0} node(s): {1}".format(
                len(error_nodes),
                ', '.join(
    ...
    def start_stop_resp(cls, **kwargs):
        logger.info(
            "RPC method start_stop_resp received: %s" %
            jsonutils.dumps(kwargs)
        )
        task_uuid = kwargs.get('task_uuid')
        nodes = kwargs.get('nodes') or []
    ...
    @classmethod
    def rollback_role_status(cls,task):
         task_cache=cls.get_task_cache(task)
         startrole=task_cache["role"]
         roleaction=task_cache[task_cache["role"].lower()]["action"]
         cluster_rolestatus=db().query(ClusterRoleStatus).filter_by(cluster_id=task.cluster.id,cluster_role=startrole).first()
         rollstatus=1
         if roleaction == "started": #onest第二次及其以后启动失败
            rollstatus=3
         elif roleaction =="stop":   #停止失败
            rollstatus=2
         objects.ClusterRoleStatusObject.update(cluster_rolestatus, data={'role_status': rollstatus})
    ...
    def _success_action(cls, task, status, progress):
        ...
        if task.cluster.mode in ('singlenode', 'multinode'):
            ...
            try:
                ...
                dashboard_url=objects.Cluster.get_network_manager(task.cluster).get_horizon_url(task.cluster.id)
                if task.cluster.cluster_type==2:
                   cluster_typename="CloudMaster"
                elif task.cluster.cluster_type==3:
                   cluster_typename="EBS"
                   setting=cls.get_cluster_setting(task.cluster)
                   horizon_ip=setting['management']['serverip']
    ...
    @classmethod
    def get_cluster_setting(cls,cluster):
        cluster_setting= jsonutils.loads(cluster.cluster_setting['cluster_setting'])
        return cluster_setting

    @classmethod
    def get_task_cache(cls,task):
        #['deployment_info']是节点列表信息,可以包含多个节点.
        return task.cache["args"]['deployment_info'][0]
    ...
    @classmethod
    def _remove_cluster_relationdata(cls,cluster_id):
        #删除cluster_deploy_msg表和cluster_role_status表和cluster_setting_info表
        clustersettings = db().query(ClusterSetting).filter_by(cluster_id=cluster_id)
        map(db().delete, clustersettings)
        db().flush()

        clusterroles = db().query(ClusterRoleStatus).filter_by(cluster_id=cluster_id)
        map(db().delete, clusterroles)
        db().flush()
       
        clusterdeploymsgs = db().query(ClusterdeployMsg).filter_by(cluster_id=cluster_id)
        map(db().delete, clusterdeploymsgs)
        db().flush()
