...
from nailgun.common.clusterstartRole import ClusterStartRole
...
class ApplyChangesTaskManager(TaskManager):
    ...
    def execute(self):
        ...
        if nodes_to_deploy:
            ...
            cluster_roles=[]
            rolelist=objects.RoleCollection.filter_by(release_id=self.cluster.release_id,cluster_type=self.cluster.cluster_type)
            for role in rolelist:
                cluster_roles.append(role.name)

            for item in deploymsg['args']['deployment_info']:
                item[ClusterStartRole.Rolename]['action']=ClusterStartRole.action
                cluster_roles.remove(ClusterStartRole.Rolename)
                logger.info(cluster_roles)
                for role in cluster_roles:
                    if item.get(role)!=None:
                       item[role]['label']="1"                
